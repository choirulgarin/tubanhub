...
import { parseCommunityPayload } from '@/lib/validators/community';
...
export async function PATCH(
  ...
  const parsed = parseCommunityPayload(body);
  if ('error' in parsed) {
...
export async function DELETE(
  ...
  const admin = createAdminClient();
  const { error } = await admin.from('communities').delete().eq('id', params.id);
  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
  return NextResponse.json({ ok: true });
